...
{
#uniongentri.fun generate the 3d  or 2d vectors  using the  3 different orders
...
if (is.null(posz))
{
	...
	if (PA==TRUE)
	{
		...
		bixy<-genbiPos.fun(posx=posx,posy=posy, 
			info=info,PA=PA,procName=procName[c(1,2)],...)
		biyx<-genbiPos.fun(posx=posy,posy=posx, 
...
#iXunion (iYunion) contains x (y) coordinates of the 2d points of the union of all sets of close points
	...
	if (PA==TRUE)
	{
		...
		biyx<-genbiPos.fun(posx=posy,posy=posx, 
			info=info,PA=PA,procName=procName[c(2,1)],...)
		bixz<-genbiPos.fun(posx=posx,posy=posz, 
			info=info,PA=PA,procName=procName[c(1,3)],...)
		bizx<-genbiPos.fun(posx=posz,posy=posx, 
			info=info,PA=PA,procName=procName[c(3,1)],...)
		biyz<-genbiPos.fun(posx=posy,posy=posz,
			info=info,PA=PA,procName=procName[c(2,3)],...)
		bizy<-genbiPos.fun(posx=posz,posy=posy,
			info=info,PA=PA,procName=procName[c(3,2)],...)

		Axyz<-gentriPos.fun(dataXY=bixy, dataYZ=biyz,info=info,
			procName=procName,...)
		Unoxyz<-Axyz[,4]*1000000+Axyz[,5]*1000+Axyz[,6]
		Axzy<-gentriPos.fun(dataXY=bixz, dataYZ=bizy,info=info,
			procName=procName[c(1,3,2)],...)
		Unoxzy<-Axzy[,4]*1000000+Axzy[,6]*1000+Axzy[,5]
		Ayxz<-gentriPos.fun(dataXY=biyx, dataYZ=bixz, info=info,
			procName=procName[c(2,1,3)],...)
		Unoyxz<-Ayxz[,5]*1000000+Ayxz[,4]*1000+Ayxz[,6]
		Ayzx<-gentriPos.fun(dataXY=biyz, dataYZ=bizx, info=info,
			procName=procName[c(2,3,1)],...)
		Unoyzx<-Ayzx[,6]*1000000+Ayzx[,4]*1000+Ayzx[,5]
		Azxy<-gentriPos.fun(dataXY=bizx, dataYZ=bixy,info=info,
			procName=procName[c(3,1,2)],...)
		Unozxy<-Azxy[,5]*1000000+Azxy[,6]*1000+Azxy[,4]
		Azyx<-gentriPos.fun(dataXY=bizy, dataYZ=biyx,info=info,
