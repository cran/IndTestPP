...
function(posx, NumProcess=2,type='Poisson',
		...
		PA=FALSE,info=FALSE,...) 
{
#DistSim.fun  generates the second and following  processes, and given the first process
...
#for each point X of the  first process
	...
	if(type=='Poisson')
	{
		posyNH<-simNHPc.fun(lambda=lambdaMarg[,1])$posNH
		if (NumProcess==3) poszNH<-simNHPc.fun(lambda=lambdaMarg[,2])$posNH
		else poszNH<-NULL
	}
